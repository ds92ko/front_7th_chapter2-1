...
`,f=({title:e,image:t,lprice:n,productId:r})=>`
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden product-card" data-product-id="${r}">
      <!-- 상품 이미지 -->
        ...
        ${t?`
          ...
          `:`
            ...
            <div class="mb-4 text-sm text-gray-600">
              총 <span class="font-medium text-gray-900">${e.length}개</span>의 상품
            </div>
            <!-- 상품 그리드 -->
            <div class="grid grid-cols-2 gap-4 mb-6" id="products-grid">
              ${e.map(f).join(``)}
  ...
  `,g=({product:e,loading:t})=>h({children:t?`
    ...
    `}),_=({filters:e,pagination:t,products:n,loading:r})=>h({children:`
      ${m({filters:e,pagination:t,loading:r})}
      ${p({products:n,loading:r})}
    `}),v=()=>h({children:`
  ...
  `,A=()=>`
    ...
    <br />
    ${k}
  `,j=`/front_7th_chapter2-1`,M=()=>r(async()=>{let{worker:e}=await import(`./browser-HvLT4yo2.js`);return{worker:e}},[]).then(({worker:e})=>e.start({serviceWorker:{url:`${j}/mockServiceWorker.js`},onUnhandledRequest:`bypass`})),N=async()=>{let e=document.getElementById(`root`);if(location.pathname===`${j}/`){e.innerHTML=_({loading:!0});let t=await i();e.innerHTML=_({...t,loading:!1})}else if(location.pathname.startsWith(`${j}/products/`)){e.innerHTML=g({loading:!0});let t=location.pathname.split(`/`).pop();try{let n=await a(t);if(n.error)throw Error(n.error);e.innerHTML=g({product:n,loading:!1})}catch{e.innerHTML=v()}}else location.pathname===`${j}/template`?e.innerHTML=A():e.innerHTML=v()};function P(){let e={push:e=>{history.pushState({},``,e),N()}};document.body.addEventListener(`click`,t=>{if(t.target.closest(`.product-card`)){let n=t.target.closest(`.product-card`).dataset.productId;e.push(`/products/${n}`)}}),N(),window.addEventListener(`popstate`,N)}M().then(P);