...
`,f=({title:e,image:t,lprice:n,productId:r})=>`
  ...
  `,A=()=>`
    ...
    <br />
    ${k}
  `,j=()=>r(async()=>{let{worker:e}=await import(`./browser-HvLT4yo2.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`})),M=async()=>{let e=document.getElementById(`root`);if(location.pathname===`/`){e.innerHTML=_({loading:!0});let t=await i();e.innerHTML=_({...t,loading:!1})}else if(location.pathname.startsWith(`/products/`)){e.innerHTML=g({loading:!0});let t=location.pathname.split(`/`).pop();try{let n=await a(t);if(n.error)throw Error(n.error);e.innerHTML=g({product:n,loading:!1})}catch{e.innerHTML=v()}}else location.pathname===`/template`?e.innerHTML=A():e.innerHTML=v()};function N(){let e={push:e=>{history.pushState({},``,e),M()}};document.body.addEventListener(`click`,t=>{if(t.target.closest(`.product-card`)){let n=t.target.closest(`.product-card`).dataset.productId;e.push(`/products/${n}`)}}),M(),window.addEventListener(`popstate`,M)}j().then(N);